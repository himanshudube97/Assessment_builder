...
/**
 * Embeddable Assessment Page
 * Stripped-down version of /a/[id] designed for iframe embedding
 */
...
export default function EmbedAssessmentPage({ params }: PageProps) {
  return (
    <Suspense fallback={<LoadingState />}>
      <EmbedAssessmentContent params={params} />
    </Suspense>
  );
}

function EmbedAssessmentContent({ params }: PageProps) {
  const { id } = use(params);
  ...
  if (error === 'invite_required') return <InviteRequiredState />;
  if (error === 'not_found') return <NotFoundState />;
  if (error === 'closed') return <ClosedState />;
  if (error === 'scheduled') return <ScheduledState openAt={scheduledOpenAt} />;
